...
<form method="post" action="member!sendsms.action" ">
		  ...
		  <div class="pageFormContent" >
                ...
                <#list memberPhone as xx>
               	 <#if xx.id != member.id >
                		<#if xx.smsint < 5 >
								<input name="sendPhoneNum" value="${xx.id}" type="checkbox" checked>
							<#else>
								<input name="sendPhoneNum" value="${xx.id}" type="checkbox" disabled=true>
						</#if>
						${xx.uname!}
					</#if>
				</#list>             
			<div class="divider"></div>   
				<label>短信内容:</label>
                <textarea name="mcsEntity.content" id="mcsEntity.content" cols="60" rows="10"></textarea>
            <div class="divider"></div>   
		</div>
		<div class="formBar">
			<ul>
