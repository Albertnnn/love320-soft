...
import cms.service.ov.TypeListOV;

@Component
public class GeneralArcTypeLabel implements LabelInterface {
	...
	public String getIterativeString(TagLabel tagLabel,
		...
		addTypeList(tagLabel.getRow(), Restrictions.eq("reid",
				Integer.parseInt(pfValue("typeId" ,filters))),Restrictions.eq("ishidden",0));
		
		//获取栏目对象
		Arctype typeEntity = atm.getArctypeManager().getArctype(new Long(Integer.parseInt(pfValue("typeId" ,filters))));
		//如果栏目下无子栏目，则显示同级栏目,而且栏目不为顶级栏目
		if( (page.getTotalCount() == 0)&&(typeEntity.getReid() != 0) ){
			addTypeList(tagLabel.getRow(), Restrictions.eq("reid",typeEntity.getReid()),Restrictions.eq("ishidden", 0));
		}
