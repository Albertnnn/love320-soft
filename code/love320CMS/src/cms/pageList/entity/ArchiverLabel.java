...
@Component
@Scope("prototype")
public class ArchiverLabel implements LabelInterface {
	...
	public String getIterativeString(TagLabel tagLabel,
		...
		// 当前栏目的子栏目ID字符串
		List<Arctype> arctypeList = atm.getArctypeManager().getAdviceList(
				new Long(tagLabel.getTypeid()));
			...
			if (arcattStr != null) {
				...
				if (tagLabel.getTypeid() == 0) {
					addArchivesList(
							...
							Restrictions
									.sqlRestriction("archives_id in  ( select archives_id from archives_arcatt where arcatt_id in("
											+ arcattStr + ") )"));
				} else {
					addArchivesList(
							tagLabel.getRow(),
							...
							tagLabel.getOrderby(),
							tagLabel.getOrderbyType(),
							Restrictions.sqlRestriction("typeid in ("
									+ typeidStr + ")"),
							Restrictions.ge("arcrank", 0),
							Restrictions
									.sqlRestriction("archives_id in  ( select archives_id from archives_arcatt where arcatt_id in("
				...
				if (tagLabel.getTypeid() == 0) {
					...
					addArchivesList(tagLabel.getRow(), 1,
							tagLabel.getOrderby(), tagLabel.getOrderbyType(),
							Restrictions.ge("arcrank", 0), Restrictions
									.sqlRestriction("typeid in (" + typeidStr
											+ ")"));
				}
